...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.2.0', __( 'WooCommerce Tab Manager', 'woocommerce-tab-manager' ), __FILE__, 'init_woocommerce_tab_manager', array( 'minimum_wc_version' => '2.3.6', 'backwards_compatible' => '4.2.0' ) );
...
class WC_Tab_Manager extends SV_WC_Plugin {
	...
	private function includes() {
		...
		if ( ! class_exists( 'WC_Tab_Manager_Search' ) ) {
			require_once( $this->get_plugin_path() . '/includes/class-wc-tab-manager-search.php' );
			$this->search = new WC_Tab_Manager_Search();
		}
	}
	...
	private function admin_includes() {
		...
		if ( ! class_exists( 'WC_Tab_Manager_Settings' ) ) {
			require_once( $this->get_plugin_path() . '/includes/class-wc-tab-manager-settings.php' );
			$this->wc_settings = new WC_Tab_Manager_Settings();
		}
	}
	...
	private function ajax_includes() {
		if ( ! class_exists( 'WC_Tab_Manager_Ajax_Events' ) ) {
			require_once( $this->get_plugin_path() . '/includes/class-wc-tab-manager-ajax-events.php' );
			$this->ajax = new WC_Tab_Manager_Ajax_Events();
		}
	}
	...
	public function batch_update_products( array $args = array() ) {
		...
		if ( 1 === $step ) {
			$this->search->update_relevanssi_searchable_tab_meta();
			$this->search->maybe_build_relevanssi_index();
		}
		...
		foreach ( $products as $product ) {
			...
			if ( ! empty( $tabs ) ) {
				...
				$this->search->update_products_for_tabs( $tab_id_list, $args );

				// Also update the Relevanssi index for the current product if
				// the plugin is installed.
				$this->search->update_relevanssi_index_for_product( $product->ID );
			}
		}
