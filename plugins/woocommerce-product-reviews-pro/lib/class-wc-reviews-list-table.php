...
class WC_Reviews_List_Table extends WP_List_Table {
	...
	public function restrict_manage_reviews() {
		...
		?>
		<input type="hidden" class="wc-product-search" name="p" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'wc-product-reviews-pro' ); ?>" data-selected="<?php echo esc_attr( $current_product_name ); ?>" value="<?php echo esc_attr( $current_product ); ?>" data-action="woocommerce_json_search_products" data-allow_clear="true" >
		<?php
	}
	...
	public function no_items() {
		...
		if ( 'moderated' === $comment_status ) {
			_e( 'No reviews awaiting moderation.', 'woocommerce-product-reviews-pro' );
		} else {
			_e( 'No reviews found.', 'woocommerce-product-reviews-pro' );
		}
	}
	...
	public function column_comment( $comment ) {
		...
		<div class="submitted-on">
			...
			printf( __( 'On %1$s%2$s at %3$s%4$s', 'wc-product-reviews-pro' ),
				'<a href="' . $comment_url . '">',
				'</a>',
		...
		<?php if ( $rating && 'yes' === get_option( 'woocommerce_enable_review_rating' ) ) :
			...
			<span class="star-rating" title="<?php /* translators: Placeholder %d - rating (e.g. 'Rated 4 out of 5') */
				echo esc_attr( sprintf( __( 'Rated %d out of 5', 'wc-product-reviews-pro' ), $rating ) ); ?>">
				<span style="width:<?php echo esc_attr( ( $rating / 5 ) * 100 ); ?>%">
					<?php printf( /* translators: Placeholder %d - rating (e.g. '4 out of 5') */
					__( '%d out of 5', 'wc-product-reviews-pro' ), $rating ) ; ?>
				</span>
			</span>
		...
		<?php if ( strlen( $full_comment ) !== strlen( $trimmed_comment ) ) : ?>
			...
			<div class="comment-text comment-text-trimmed">
				<?php echo $trimmed_comment; ?> <a href="#" class="comment-show-more-link"><?php esc_html_e( 'show more', 'wc-product-reviews-pro' ); ?></a>
			</div>
