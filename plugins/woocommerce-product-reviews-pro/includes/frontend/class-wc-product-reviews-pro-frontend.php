...
class WC_Product_Reviews_Pro_Frontend {
	...
	public function __construct() {
		...
		// Add my contributions table to the my account area
		add_action( 'woocommerce_before_my_account', array( $this, 'render_my_account_contributions' ), 11 );
	 ...
	 * @return int|string Return 1 (truthy) if manual contribution moderation is on, empty (falsy) otherwise
	 */
	 ...
	 * @since 1.2.0
	 */
	public function render_my_account_contributions() {

		// Get the comments for the user
		$id = get_current_user_id();
		$comments = get_comments( array( 'user_id' => $id ) );

		// We'll pass in post types the comments have been left on so we can display comments only on products
		$comments_on = array();

		foreach ( $comments as $comment ) {
			$comment_post_type = get_post_type( $comment->comment_post_ID );
			array_push( $comments_on, $comment_post_type);
		}
		...
		if ( ! empty( $enabled_contribution_types ) ) {
			...
			wc_get_template(
				...
				'',
				wc_product_reviews_pro()->get_plugin_path() . '/templates/'
			);
		}
	}
